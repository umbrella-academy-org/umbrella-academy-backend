...
router.put('/profile', authenticate, requireRole('student'), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { fieldId, trainerId, educationLevel, availability, learningPreferences } = req.body;
...
router.put('/:id/status', authenticate, requireRole('company-admin', 'umbrella-admin'), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const updated = await User.findByIdAndUpdate(req.params.id, { status }, { new: true, runValidators: true }).select('-password');
    if (!updated) { res.status(404).json({ success: false, message: 'User not found' }); return; }
    res.json({ success: true, data: updated });
  } catch (err) { next(err); }
});
...
router.post('/', authenticate, requireRole('umbrella-admin', 'company-admin'), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!email || !password || !role || !firstName || !lastName) {
      res.status(400).json({ success: false, message: 'email, password, role, firstName, and lastName are required' }); return;
    }
